...
export function TradeDashboard() {
    ...
    for (let i = 0; i < orderbookAsks.length; i++) {
        ...
        if (i === 0) {
            ...
            orderbookAsks[i].td1.toPrecision(6) === askBins[lastAskIndex].td1.toPrecision(6)
        ) {
    ...
    const bidBins = [];
    for (const i in orderbookBids) {
        const lastBidIndex = bidBins.length - 1; // => -1
        if (i === '0') {
            bidBins.push(orderbookBids[i]);
        } else if (
            orderbookBids[i].td1.toPrecision(6) === bidBins[lastBidIndex].td1.toPrecision(6)
        ) {
