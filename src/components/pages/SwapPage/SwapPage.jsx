...
export default function SwapPage() {
  ...
  const isSwapCompatible = useMemo(() => network && api.isImplemented('depositL2'), [network])
      ...
      <div className="swap_section">
        <div className="swap_container">
          <div className="swap_head_tabs">
            <Link className={cx({ 'swap_head_tab_active': tab === 'swap' })} to="/swap">Swap</Link>
            <Link className={cx({ 'swap_head_tab_active': tab === 'receipts' })} to="/swap/receipts">Receipts</Link>
          </div>
        </div>
        <div className="swap_container" style={{ flex: '1 1 auto' }}>
          {isSwapCompatible
            ? tab === 'swap' ? <Swap /> : <SwapReceipts />
            : <SwapIncompatible />}
        </div>
      </div>
    </BridgeTemplate>
