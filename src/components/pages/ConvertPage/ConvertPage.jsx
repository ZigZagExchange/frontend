...
const ConvertPage = () => {
    ...
    const estimatedValueSell =
        sellAmounts * coinEstimator(sellToken?.name) || 0;
    ...
    const getBaseAllowance = () => {
        if (!marketInfo) return 0;
        if (!balances?.[marketInfo.baseAsset.symbol]?.allowanceReadable)
            return 0;
    ...
    const getQuoteAllowance = () => {
        if (!marketInfo) return 0;
        if (!balances?.[marketInfo.quoteAsset.symbol]?.allowanceReadable)
            return 0;
    ...
    const fromTokenOptions = useMemo(() => {
        if (sellTokenList.length > 0) {
            ...
            if (!sellToken) {
                const f = s.find(
                    (item) => item.name === currentMarket.split("-")[1]
                );
                setSellToken(f);
            }
        ...
        if (tType === "sell") {
            ...
            if (api.isEVMChain()) {
                ...
                if (quoteAmount + fee < allowance) {
                    setApproveNeeded(true);
                    setError(
                        `Amount exceeds ${marketInfo.quoteAsset.symbol} allowance`
                    );
                    return;
                } else {
                    setApproveNeeded(false);
    ...
    const onClickExchange = async (e) => {
        ...
        baseAmount =
            tType === "buy" ? baseAmount : baseAmount + getBaseFee(baseAmount);
        quoteAmount =
            tType === "buy"
                ? quoteAmount + getQuoteFee(quoteAmount)
                : quoteAmount;
            ...
            return (
                <div>
                    ...
                    <p style={{ fontSize: "14px", lineHeight: "24px" }}>
                        ...
                        {tType === "sell"
                            ? `${addComma(
                                  formatPrice(getBaseFee(baseAmount))
                              )} ${marketInfo.baseAsset.symbol}`
                            : `${addComma(
                                  formatPrice(getQuoteFee(quoteAmount))
                              )} ${marketInfo.quoteAsset.symbol}`}
                    </p>
                    <p style={{ fontSize: "14px", lineHeight: "24px" }}>
            ...
            {loading && (
                <div
                    className={classNames(
                        "flex justify-center align-center mt-48",
                        {
                            dark: isDark,
                        }
                    )}
