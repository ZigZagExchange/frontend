...
const ConvertPage = () => {
    ...
    useEffect(() => {
        const fee =
            tType === 'sell' ? getBaseFee(Number(buyAmounts)) : getQuoteFee(Number(sellAmounts));
    ...
    useEffect(() => {
        ...
        price *= tType === 'buy' ? 1 + slippageValue / 100 : 1 - slippageValue / 100;
    ...
    useEffect(() => {
        if (changedBuyAmount) {
            const newSellAmount =
                tType === 'sell' ? buyAmounts / currentPrice : buyAmounts * currentPrice;
            setBuyAmounts(newSellAmount);
        }

        if (changedSellAmount) {
            const newBuyAmount =
                tType === 'sell' ? sellAmounts * currentPrice : sellAmounts / currentPrice;
    ...
    const getBaseBalance = () => {
        ...
        Object.keys(userOrders).forEach((orderId) => {
            ...
            const sellToken = order[3] === 's' ? order[2].split('-')[0] : order[2].split('-')[1];
    ...
    const getBaseFee = (amount) => {
        ...
        fee += marketInfo.makerVolumeFee && amount ? amount * marketInfo.makerVolumeFee : 0;
        return fee;
    };
        ...
        if (sellTokenList.length > 0) {
            const p = sellTokenList.map((item, index) => {
                const price = balances[item]?.valueReadable
                    ? `$ ${formatUSD(coinEstimator(item) * balances[item]?.valueReadable)}`
                    : '';
                return {
                    id: index,
                    name: item,
    ...
    const buyTokenOptions = useMemo(() => {
        ...
        if (buyToken) {
            ...
            const b = filtered.find((item) => item.name === currentMarket.split('-')[0]);
            setBuyToken(b);
        }
    ...
    const onChangeSellAmounts = (event) => {
        ...
        const newBuyAmount = tType === 'buy' ? amount / currentPrice : amount * currentPrice;
        setBuyAmounts(newBuyAmount);
        setChangedBuyAmount(false);
    ...
    const onChangeBuyAmounts = (event) => {
        ...
        const newSellAmount = tType === 'buy' ? amount * currentPrice : amount / currentPrice;
    ...
    const isValid = () => {
        ...
        const buyAmountParsed =
            typeof buyAmounts === 'string' ? parseFloat(buyAmounts.replace(',', '.')) : buyAmounts;
        let baseAmount = baseToken === buyToken?.name ? buyAmountParsed : sellAmountParsed;
        let quoteAmount = quoteToken === sellToken?.name ? sellAmountParsed : buyAmountParsed;
    ...
    const approveHandler = async (e) => {
        ...
        const token = tType === 'sell' ? marketInfo.baseAsset.symbol : marketInfo.quoteAsset.symbol;

        let orderApproveToast = toast.info('Approve pending. Sign or Cancel to continue...', {
    ...
    const onClickMax = () => {
        ...
        if (balance && fees) {
            ...
            if (s_amounts < 0) {
                ...
                const newBuyAmount =
                    tType === 'sell' ? s_amounts * currentPrice : s_amounts / currentPrice;
                setBuyAmounts(newBuyAmount);
            }
        }
        setChangedBuyAmount(false);
            ...
            {!loading && (
                <div className={classNames('flex justify-center', { dark: isDark })}>
                    <div className="w-full max-w-lg px-1 sm:px-0">
                        ...
                        />
                        {!errorMsg && user.address && !approveNeeded && Number(sellAmounts) > 0 && (
                            <Button
                                isLoading={false}
