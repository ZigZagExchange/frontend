...
class SpotForm extends React.Component {
    ...
    getQuoteBalance() {
        ...
        if (!marketInfo) return 0;
        if (
            !this.props.balances?.[marketInfo.zigzagChainId]?.[marketInfo.quoteAsset.symbol]
                ?.valueReadable
        )
            return 0;
        let totalBalance =
    ...
    async approveHandler(e) {
        ...
        const marketInfo = this.props.marketInfo;
        const token =
            this.props.side === 's' ? marketInfo.baseAsset.symbol : marketInfo.quoteAsset.symbol;

        let newstate = { ...this.state };
        this.setState(newstate);
        let orderApproveToast = toast.info('Approve pending. Confirm or Reject to continue...', {
    ...
    async buySellHandler(e) {
        ...
        if (this.props.side === 's') {
            ...
            if (this.state.baseAmount < fee) {
                ...
                toast.error(`Minimum order size is ${fee} ${marketInfo.baseAsset.symbol}`);
                return;
            }
            ...
            if (this.state.quoteAmount < fee) {
                fee = Number(fee).toPrecision(5);
                toast.error(`Minimum order size is ${fee} ${marketInfo.quoteAsset.symbol}`, {
    ...
    componentDidUpdate(prevProps, prevState) {
        ...
        if (this.props.orderType !== prevProps.orderType || this.props.side !== prevProps.side) {
            const newState = {
                ...this.state,
                baseAmount: '',
                quoteAmount: '',
            ...
            if (marketInfo) {
                ...
                feeToken =
                    this.props.side === 'b'
                        ...
                        : marketInfo.baseAsset.symbol;
                const amount =
                    this.state.side === 'b' ? this.state.quoteAmount : this.state.baseAmount;
    ...
    render() {
        ...
        if (isNaN(baseBalance) || (marketInfo?.baseFee && baseBalance < marketInfo.baseFee)) {
            // hide small balances
            baseBalance = 0;
        }
        if (isNaN(quoteBalance) || (marketInfo?.quoteFee && quoteBalance < marketInfo.quoteFee)) {
            // hide small balances
            quoteBalance = 0;
        }
        ...
        const exchangePercentage = this.getExchangePercentage(baseAmount, quoteAmount);
        ...
        const showMinusBox =
            !this.props.user.id || this.state.baseAmount === '' || this.state.baseAmount <= 0;
                ...
                <StyledForm isMobile={isMobile}>
                    <InputBox>
                        ...
                        <InputField
                            type="text"
                            pattern="\d+(?:[.,]\d+)?"
                            placeholder={`Price (${marketInfo && marketInfo.baseAsset?.symbol}-${
                                marketInfo && marketInfo.quoteAsset?.symbol
                            })`}
                            value={
                                this.props.orderType === 'limit'
