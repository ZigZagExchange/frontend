...
class SpotForm extends React.Component {
    ...
    getExchangePercentage(baseAmount, quoteAmount) {
        ...
        if (this.props.side === "s") {
            ...
            result =
                ((Number(baseAmount) + Number(baseFee)) / baseBalance) * 100;
        } else {
            const quoteBalance = this.getQuoteBalance();
            const quoteFee = this.getQuoteFee();
            result =
                ((Number(quoteAmount) + Number(quoteFee)) / quoteBalance) * 100;
        }
    ...
    updateBaseAmount(e) {
        ...
        if (
            ...
            const fee = this.getBaseFee(newState.baseAmount);
            newState.quoteAmount =
                this.currentPrice() * (newState.baseAmount - fee);
            if (newState.quoteAmount < 0) {
                newState.quoteAmount = "";
                newState.baseAmount = "";
            }
    ...
    updateQuoteAmount(e) {
        ...
        if (
            ...
            const fee = this.getQuoteFee(newState.quoteAmount);
            newState.baseAmount =
                (newState.quoteAmount - fee) / this.currentPrice();
            if (newState.baseAmount < 0) {
                newState.quoteAmount = "";
                newState.baseAmount = "";
            }
    ...
    increaseAmount(e) {
        ...
        if (this.props.side === "b") {
            ...
            const newBaseAmount =
                Number(this.state.baseAmount) + baseBalance * 0.02;
            newState.baseAmount =
                newBaseAmount > baseBalance ? baseBalance : newBaseAmount;
            const fee = this.getBaseFee(newState.baseAmount);
            newState.quoteAmount =
                (newState.baseAmount - fee) * this.currentPrice();
            if (newState.quoteAmount < 0) {
                newState.baseAmount = "";
                newState.quoteAmount = "";
            }
        ...
        const renderGuidContent = () => {
            return (
                <div>
                    ...
                    <p style={{ fontSize: "14px", lineHeight: "24px" }}>
                        {addComma(formatPrice(baseAmount))}{" "}
                        {marketInfo.baseAsset.symbol} @{" "}
    ...
    rangeSliderHandler(e, val) {
        ...
        if (this.props.side === "s") {
            ...
            newstate.quoteAmount =
                amount > quoteBalance ? quoteBalance : amount;
            const fee = this.getQuoteFee(newstate.quoteAmount);
            newstate.baseAmount =
                (newstate.quoteAmount - fee) / this.currentPrice();
            if (newstate.baseAmount < 0) {
    ...
    componentDidUpdate(prevProps, prevState) {
        ...
        if (
            ...
            prevProps.marketInfo &&
            this.props.marketInfo !== prevProps.marketInfo
        ) {
            const newState = { ...this.state };
            if (this.props.side === "s" && !Number.isNaN(newState.baseAmount)) {
                // follow fee for sell order
                const newBaseAmount =
                    Number(newState.baseAmount) +
                    this.props.marketInfo.baseFee -
                    prevProps.marketInfo.baseFee;
                ...
                this.props.side === "b" &&
                !Number.isNaN(this.props.quoteChanged)
            ) {
                // follow fee for buy order
                const newQuoteAmount =
                    Number(newState.quoteAmount) +
                    this.props.marketInfo.quoteFee -
                    prevProps.marketInfo.quoteFee;
        ...
        if (this.props.orderType !== prevProps.orderType) {
            ...
            if (this.props.side === "b") {
                // for buy quoteAmount should be fixed
                newState.baseAmount =
                    newState.quoteAmount / this.currentPrice();
                newState.baseAmount =
                            ...
                            value={
                                this.state.baseAmount !== ""
                                    ? this.state.baseChanged
                                        ? this.state.baseAmount
                                        : addComma(
                                              formatPrice(this.state.baseAmount)
                                          )
                                    : ""
                            }
