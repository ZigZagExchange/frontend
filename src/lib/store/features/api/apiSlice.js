    ...
    reducers: {
            ...
            (payload[0] || []).forEach(async (update) => {
                ...
                switch (newStatus) {
                    ...
                    case "m":
                        ...
                        const orderUserId =
                            state.userId &&
                            state.userId.toString().toLowerCase();
                        delete state.orders[orderId];
                        if (
                            matchedOrder &&
                            (takerUserId === orderUserId ||
                                makerUserId === orderUserId)
                        ) {
                    ...
                    case "r":
                        ...
                        if (filledOrder) {
                            ...
                            filledOrder[11] = txHash;
                            const noFeeOrder =
                                api.getOrderDetailsWithoutFee(filledOrder);
                            toast.error(
                                ...
                                } was rejected: ${error}`,
                                {
                                    toastId: `Your ${sideText} order for ${
                                        noFeeOrder.baseQuantity.toPrecision(4) /
                                        1
                                    } ${baseCurrency} @ ${
                                        noFeeOrder.price.toPrecision(4) / 1
                                    } was rejected: ${error}`,
            ...
            const renderToastContent = () => {
                return (
                    <div>
                        ...
                        {ethWallet &&
                            renderBridgeLink(ethWallet.text, ethWallet.link)}
                    </div>
                );
            };
        ...
        setHighSlippageModal(state, { payload }) {
            state.highSlippageModal = {
                ...
                delta: payload.delta
                    ? payload.delta
                    : state.highSlippageModal.delta,
                type: payload.type
                    ? payload.type
                    : state.highSlippageModal.type,
