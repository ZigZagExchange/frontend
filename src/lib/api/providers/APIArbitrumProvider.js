...
export default class APIArbitrumProvider extends APIProvider {
    ...
    getAccountState = async () => this.accountState;
    ...
    getAllowance = async (tokenAddress, contractAddress) => {
        ...
        const allowance = await erc20Contract.allowance(this.accountState.address, contractAddress);
    ...
    approveExchangeContract = async (token, amount) => {
        ...
        const signer = await this.api.rollupProvider.getSigner();
        const erc20Contract = new ethers.Contract(currencyInfo.address, erc20ContractABI, signer);
    ...
    warpETH = async (amountBN) => {
        ...
        const signer = await this.api.rollupProvider.getSigner();
        const wethContract = new ethers.Contract(wethInfo.address, wethContractABI, signer);
    ...
    getWrapFees = async () => {
        ...
        const feeResult = ethers.utils.formatEther(feeData.gasPrice.mul(450_000)).toString();
        return {
            wrap: feeResult,
            unwrap: feeResult,
        };
