...
export default class APIZKProvider extends APIProvider {
    ...
    getBalances = async () => {
        ...
        this.api.getCurrencies().forEach((ticker) => {
            ...
            balances[ticker] = {
                value: balance,
                valueReadable:
                    (balance && currencyInfo && balance / 10 ** currencyInfo.decimals) || 0,
    ...
    getAccountState = async () => (this.syncWallet ? this.syncWallet.getAccountState() : {});
    ...
    withdrawL2FastBridgeFee = async (token) => {
         ...
         * */
        const currencyInfo = this.api.getCurrencyInfo(token);
        const getNumberFormatted = (atoms) => parseInt(atoms) / 10 ** currencyInfo.decimals;
        ...
        if (this.api.rollupProvider) {
            ...
            const bridgeFee = feeData.maxFeePerGas.add(feeData.maxPriorityFeePerGas).mul(21000);

            if (token === 'ETH') {
                return getNumberFormatted(bridgeFee);
            }
            if (['FRAX', 'UST'].includes(token)) {
    ...
    getSeedKey = async (ethSigner) => `${this.network}-${await ethSigner.getAddress()}`;

    getSeed = async (ethSigner) => {
        const seedKey = await this.getSeedKey(ethSigner);
        const seeds = this.getSeeds(ethSigner);
    ...
    signMessage = async (message) => await this.ethWallet.signMessage(message);
