...
export default class APIStarknetProvider extends APIProvider {
    ...
    getAccountState = async () => this._accountState;
    ...
    submitOrder = async (product, side, price, baseAmount, quoteAmount) => {
        ...
        const tokenAddress = this.api.currencies[sellCurrency].chain[this.network].contractAddress;
        const { decimals } = this.api.currencies[sellCurrency];
        ...
        const minAmountInt = bigInt(1e20 * 10 ** decimals);
        const amountInt = bigInt(1e21 * 10 ** decimals);
    ...
    signIn = async () => {
        ...
        if (localStorage.getItem('starknet:account')) {
            ...
            const deployContractToast = toast.info(
                ...
                {
                    autoClose: false,
                    toastId: 'First time using Zigzag Starknet. Deploying account contract...',
                },
            );
        ...
        for (const currency in committedBalances) {
    ...
    _getBalances = async (userAddress) => {
        const balances = {};
        for (const currency in this.api.currencies) {
            if (this.api.currencies[currency].chain[this.network]) {
                const { contractAddress } = this.api.currencies[currency].chain[this.network];
                if (contractAddress) {
                    const balance = await this._getBalance(contractAddress, userAddress);
    ...
    _createOrderHash = (product, side, price, amount, expiration) => {
        ...
        const decimalDifference = this.api.currencies[baseCurrency].decimals;
        ...
        const baseQuantityInt = (amount * 10 ** this.api.currencies[baseCurrency].decimals).toFixed(
            0,
        );
