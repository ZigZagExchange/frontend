...
export default class API extends Emitter {
  ...
  setAPIProvider = (network, networkChanged = true) => {
    ...
    this.web3 = new Web3(
      window.ethereum ||
        new Web3.providers.HttpProvider(`https://${chianName}.infura.io/v3/${this.infuraId}`)
    );
  ...
  start = () => {
    ...
    if (accountState && accountState.id) {
      this.send("login", [this.apiProvider.network, accountState.id && accountState.id.toString()]);
    }
  };
    ...
    if (!this._signInProgress) {
      this._signInProgress = Promise.resolve()
        .then(async () => {
          ...
          this.polygonProvider = new ethers.providers.JsonRpcProvider(this.getPolygonUrl(network));
          ...
          if (accountState && accountState.id) {
            this.send("login", [network, accountState.id && accountState.id.toString()]);
          }
  ...
  getBalanceOfCurrency = async (currency) => {
    ...
    try {
      ...
      if (netContract) {
        result.allowance = ethers.BigNumber.from(await contract.allowance(account, netContract));
      }
      return result;
    } catch (e) {
      console.log(e);
