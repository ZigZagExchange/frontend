...
export default class API extends Emitter {
    ...
    getAPIProvider = (network) => this.networks[this.getNetworkName(network)][1];
    ...
    getProfile = async (address) => {
        ...
        if (!this._profiles[address]) {
            ...
            Object.assign(
                profile,
                ...(await Promise.all([fetchENSName(address), getProfileFromIPFS(address)])),
            );
    ...
    getPolygonWethBalance = async () => {
        ...
        const p = formatAmount(wethBalance, { decimals: 18 });
    ...
    transferPolygonWeth = async (amount, walletAddress) => {
        ...
        try {
            ...
            const polygonProvider = new ethers.providers.Web3Provider(window.web3.currentProvider);

            networkSwitched = true;

            const wethContractAddress = this.getPolygonWethContract(this.apiProvider.network);

            const contract = new this.web3.eth.Contract(wethContractABI, wethContractAddress);
    ...
    getChainIdFromName = (name) => this.networks?.[name]?.[0];
    ...
    isZksyncChain = () => !!this.apiProvider.zksyncCompatible;

    isEVMChain = () => !!this.apiProvider.evmCompatible;
    ...
    depositL2 = async (amount, token, address) =>
        this.apiProvider.depositL2(amount, token, address);
    ...
    withdrawL2 = async (amount, token) => this.apiProvider.withdrawL2(amount, token);

    transferToBridge = (amount, token, address, userAddress) =>
        this.apiProvider.transferToBridge(amount, token, address, userAddress);

    depositL2Fee = async (token) => await this.apiProvider.depositL2Fee(token);
    ...
    isImplemented = (method) =>
        this.apiProvider[method] && !this.apiProvider[method].notImplemented;

    getNetworkContract = () => this.networks[this.getNetworkName(this.apiProvider.network)][2];
    ...
    getBalanceOfCurrency = async (currency) => {
        const currencyInfo = this.getCurrencyInfo(currency);
        const result = { balance: 0, allowance: ethersConstants.Zero };
    ...
    getOrderDetailsWithoutFee = (order) => {
        ...
        let quoteQuantityWithoutFee;
        let priceWithoutFee;
        let remainingWithoutFee;
        if (side === 's') {
    ...
    refreshArweaveAllocation = async (address) =>
        this.apiProvider.refreshArweaveAllocation(address);

    purchaseArweaveBytes = (bytes) => this.apiProvider.purchaseArweaveBytes(bytes);

    signMessage = async (message) => this.apiProvider.signMessage(message);

    approveExchangeContract = async (token, amount) =>
        this.apiProvider.approveExchangeContract(token, amount);
    ...
    warpETH = async (amount) => {
        ...
        const amountBN = ethers.utils.parseEther(amount.toFixed(18));
    ...
    getWrapFees = async () => this.apiProvider.getWrapFees();
    ...
    getTokenInfo = (tokenLike, chainId) => this.apiProvider.getTokenInfo(tokenLike, chainId);

    getTokenPrice = (tokenLike, chainId) => this.apiProvider.tokenPrice(tokenLike, chainId);
    ...
    getCurrencyInfo = (currency) => {
        ...
        for (let i = 0; i < pairs.length; i++) {
            ...
            if (quoteCurrency === currency && marketInfo) {
    ...
    getCurrencies = (chainId = this.apiProvider.network) => {
        ...
        for (const market in this.lastPrices[chainId]) {
